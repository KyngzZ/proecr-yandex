...
# Музыка
pygame.mixer.music.load("начало.mp3")  # Музыка для меню
game_music = "игра.mp3"  # Музыка во время игры
end_music = "конец.mp3"  # Музыка в концовке

# Звуки
gunshot_sound = pygame.mixer.Sound("одиночка.mp3")  # Звук выстрела
firework_sound = pygame.mixer.Sound("живой фирверк.mp3")  # Звук фейерверка
...
class Player(pygame.sprite.Sprite):
    ...
    def shoot(self):
        if self.is_alive():
            ...
            gunshot_sound.play()  # Воспроизведение звука выстрела
        if self.is_alive():
            projectile = Projectile(self.rect.centerx, self.rect.top, self)
            all_sprites.add(projectile)
            projectiles.add(projectile)
...
class Boss2(pygame.sprite.Sprite):
    def __init__(self):
        ...
        try:
            ...
            print("Ошибка загрузки boss2.png, используется заливка:", e)
            self.image = pygame.Surface((100, 100))
            self.image.fill((128, 0, 128))

        self.rect = self.image.get_rect(center=(WIDTH // 2, 150))
        self.health = 700  # Больше здоровья
        self.last_attack_time = pygame.time.get_ticks()
